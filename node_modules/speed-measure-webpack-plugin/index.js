...
module.exports = class SpeedMeasurePlugin {
  constructor(options) {
    // 将 new SpeedMeasurePlugin 时传入的参数赋值给 `this.options`
    // 参数列表参考：https://github.com/stephencookdev/speed-measure-webpack-plugin?tab=readme-ov-file#options
    this.options = options || {};

    this.timeEventData = {};
    this.smpPluginAdded = false;

    // `bind` 函数返回函数引用，并将函数的 this 设置为 SpeedMeasurePlugin 的实例
    // `bind` 的用法：https://developer.mozilla.org/en-US/docs/web/javascript/reference/global_objects/function/bind
  ...
  wrap(config) {
    // 如果 disable 为 true，则不执行 speed-measure-webpack-plugin，这个配置可以用在 .env 中
    // disable 的文档：https://github.com/stephencookdev/speed-measure-webpack-plugin?tab=readme-ov-file#optionsdisable
    ...
    // 如果在 webpack.config.js 中配置了 plugin 选项，则遍历 plugin 选项
    config.plugins = (config.plugins || []).map((plugin) => {
      ...
      // 实例化 WrappedPlugin，传入插件、插件名、SpeedMeasurePlugin 实例
