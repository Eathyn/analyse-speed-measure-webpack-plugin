...
module.exports.WrappedPlugin = class WrappedPlugin {
  constructor(plugin, pluginName, smp) {
    ...
    return new Proxy(plugin, {
      get(target, property) {
        // 访问插件的 apply 方法时，返回 WrappedPlugin 实例的 apply 方法
        if (property === "apply") {
          return wp.apply;
        }
  ...
  apply(compiler) {
    // 调用配置的插件的 apply 方法，例如：CleanWebpackPlugin 类的 apply 方法
    return this._smp_plugin.apply(
      // 传入 apply 的实参本来为 compiler，现在变成通过 wrap 函数的返回值，这个返回值是用 proxy 包裹起来的 compiler
